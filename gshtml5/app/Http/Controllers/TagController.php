...
class TagController extends Controller
{
   public function showTag($name)
    ...
    public function tag_a(){
        $content_etiqueta = file_get_contents('/home/ignacio/GSHTML5/docker/db/mongodb/data/a.json');
    ...
    public function tag_abbr(){
        $content_etiqueta = file_get_contents('C:\GSHtml5\docker\db\mongodb\data\abbr.json');
        $json_en_array_php = json_decode($content_etiqueta, true);

        return view("tagDetail",["tagdata"=>$json_en_array_php]);
    }
    public function tag_address(){
        $content_etiqueta = file_get_contents('C:\GSHtml5\docker\db\mongodb\data\address.json');
        $json_en_array_php = json_decode($content_etiqueta, true);

        return view("tagDetail",["tagdata"=>$json_en_array_php]);
    }
    public function tag_del(){
        $content_etiqueta = file_get_contents('C:\GSHtml5\docker\db\mongodb\data\del.json');
        $json_en_array_php = json_decode($content_etiqueta, true);
